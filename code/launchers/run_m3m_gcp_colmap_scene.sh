...
RUN_ROOT=${RUN_ROOT:-/root/autodl-tmp/runs/ms-gcp-3dgs/colmap}
GPU_ID=${GPU_ID:-0}
...
run_stage(){ local scene_id="$1"; local stage="$2"; shift 2; local scene_dir="$RUN_ROOT/$scene_id"; mkdir -p "$scene_dir/logs"; local log="$scene_dir/logs/${stage}.log"; local trace="$scene_dir/logs/${stage}_gpu_trace.csv"; event "START $scene_id $stage"; sample_gpu "$trace" & local mon=$!; set +e; CUDA_VISIBLE_DEVICES=$GPU_ID PYTHONUNBUFFERED=1 SIGS_COLMAP_EXECUTABLE="$COLMAP" "$@" > "$log" 2>&1; local rc=$?; set -e; kill "$mon" 2>/dev/null || true; wait "$mon" 2>/dev/null || true; event "END $scene_id $stage rc=$rc"; return "$rc"; }

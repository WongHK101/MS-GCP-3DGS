...
def validate(repo_root: Path, recipe_path: Path, adapter_path: Path, source: Path) -> dict[str, Any]:
    ...
    require(recipe.get("status") == "FROZEN_STATIC_PREFLIGHT_GPU_PENDING", "recipe status mismatch")
    require(adapter.get("status") == "STATIC_PATCH_PREFLIGHT_PASS_GPU_PENDING", "adapter status mismatch")
    ...
    require(execution.get("training_authorized") is False, "formal training must remain locked")
    ...
    for key in (
        ...
        "full_scene_matrix_allowed",
        "global_training_allowed",
    ):
        require(qualification.get(key) is False, f"pending gate unexpectedly passed/unlocked: {key}")
    ...
    return {
        ...
        "remaining_gates": [
            "target_gpu_official_training_extension_build",
            "target_gpu_evaluation_adapter_build",
            "real_input_loader_82_train_0_test_61302_points",
            "synthetic_raw_moment_conformance",
            "one_iteration_technical_smoke",
            "frozen_3k_66_camera_packet_and_common_evaluator",
        ],
        "errors": errors,
    }

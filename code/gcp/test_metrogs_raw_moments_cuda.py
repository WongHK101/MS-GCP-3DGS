#!/usr/bin/env python3
"""Target-GPU conformance for MetroGS's evaluation-only A/M1/M2/H."""
...
def settings(torch: Any, image_size: int = 9) -> Any:
    from dist_2dgs import GaussianRasterizationSettings
...
def rasterize(torch: Any, z_values: list[float], opacity_values: list[float]) -> Any:
    from dist_2dgs import GaussianRasterizer

    count = len(z_values)
    means3d = torch.tensor(
        [[0.0, 0.0, value] for value in z_values],
        dtype=torch.float32,
        device="cuda",
    )
    means2d_input = torch.zeros((count, 3), dtype=torch.float32, device="cuda")
    ...
    means2d, rgb, normal_opacity, radii, transmat, depths = (
    ...
    return rasterizer.render_gaussians(
        means2D=means2d,
...
def raw_peak(allmap: Any) -> dict[str, float | int]:
    ...
    if value.shape[0] != 9:
        raise AssertionError(f"patched MetroGS allmap must have 9 planes, got {value.shape}")
...
def run(eval_repo: Path) -> dict[str, Any]:
    ...
    extension = next(Path(dist_2dgs.__file__).resolve().parent.glob("_C*.so"))
    with torch.no_grad():
        single_outputs = rasterize(torch, [20.0], [0.25])
        two_outputs = rasterize(torch, [10.0, 30.0], [0.4, 0.5])
    if len(single_outputs) != 4:
    ...
    variance_check = assert_close(
        "two_variance", variance, 97.95918367346938, atol=3e-4
    )
    return {
        "schema": "m3m_gcp_native_quarter_metrogs_raw_moment_cuda_conformance_v1",
        ...
        "camera_z_definition": "per-ray 2D surfel intersection z from the frozen MetroGS rasterizer",
        ...
        "runtime": {
            ...
            "package": str(Path(dist_2dgs.__file__).resolve()),

#!/usr/bin/env python3
"""Prepare MetroGS's frozen training-only MoGe, multi-view, and Pi3 priors."""
...
def validate_training_images(
    ...
    if actual_names != expected_names:
        raise RuntimeError(
            "MetroGS image root differs from the frozen 82-view training set: "
...
def validate_segments(
    segment_root: Path, source_image_hashes: Counter[str], split_num: int
) -> list[dict[str, Any]]:
    ...
    if copied_hashes != source_image_hashes:
        raise RuntimeError("segmented image bytes are not an exact partition of the 82 training images")
    return rows
...
def main() -> int:
    ...
    if formal["scene"] != "gcp_3000_20260602":
        raise RuntimeError("MetroGS preparation is frozen to the 3K qualification scene")
    train_records = [record for record in formal["images"] if record["role"] == "train"]
    if len(train_records) != 82 or int(formal["test_view_count"]) != 12:
        raise RuntimeError("frozen 82/12 split mismatch")

#!/usr/bin/env python3
"""Build the reviewed activation-v4 successor after the MetroGS pre-child fix."""
...
PLAN = "configs/m3m_gcp_native_quarter_100k_ten_method_execution_plan_v4.json"
...
def main() -> int:
    ...
    if args.execution_plan_review_verdict != PLAN_VERDICT:
        raise RuntimeError("exact 100K successor review verdict is absent")
    ...
    if head != args.execution_plan_reviewed_commit:
        raise RuntimeError("checkout differs from successor reviewed commit")
    plan_tree = git_value(
    ...
    for key, relative in (
        ("contract", CONTRACT),
        ("schema", SCHEMA),
        ("local", LOCAL_PREPARATION),
        ("remote", REMOTE_PREPARATION),
    ):
        if (
            git_blob_sha256(repo, args.protocol_reviewed_commit, relative)
            != sha256_file(paths[key])
        ):
    ...
    if (
        ...
        raise RuntimeError("successor plan does not bind the exact Phase-1 PASS")
    if (
        plan.get("schema")

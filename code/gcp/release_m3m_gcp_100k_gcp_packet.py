...
def validate_archive(path: Path, expected_root: Path) -> dict[str, Any]:
...
def main() -> int:
    ...
    # Crash-safe completion: an immutable intent plus both targets absent is sufficient
    ...
    if intent_path.is_file():
        ...
        if packet_root.exists() or packet_root.is_symlink() or state_path.exists() or state_path.is_symlink():
            raise RuntimeError("existing GCP release intent has not completed both deletions")
        if (
    ...
    for path in (packet_root, state_path, phase_path, authorization_path, verification_path, archive_path):
        if path.is_symlink():

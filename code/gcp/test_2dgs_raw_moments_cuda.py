#!/usr/bin/env python3
"""Target-GPU conformance test for the evaluation-only 2DGS A/M1/M2/H output."""
...
def raw_peak(allmap: Any) -> dict[str, float | int]:
    ...
    if value.shape[0] != 9:
        raise AssertionError(f"patched 2DGS allmap must have 9 planes, got {value.shape}")
...
def run(eval_repo: Path) -> dict[str, Any]:
    ...
    extension = next(Path(diff_surfel_rasterization.__file__).resolve().parent.glob("_C*.so"))
    ...
    variance_check = assert_close("two_variance", variance, 685.7142857142857 / 7.0, atol=2e-4, rtol=2e-5)

    return {
        "schema": "m3m_gcp_native_quarter_2dgs_raw_moment_cuda_conformance_v1",
        ...
        "runtime": {
            ...
            "package": str(Path(diff_surfel_rasterization.__file__).resolve()),

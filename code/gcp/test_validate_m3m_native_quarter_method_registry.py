...
def test_native_quarter_registry_passes() -> None:
    ...
    assert result["training_allowed_methods"] == ["gof"]
...
def test_3dgs_formal_3k_result_is_complete_without_unlocking_the_matrix() -> None:
    ...
    assert value["per_method_training_allowed_methods"] == ["gof"]
...
def test_gof_gpu_qualification_unlocks_only_one_3k_run() -> None:
    value = json.loads(REGISTRY.read_text(encoding="utf-8"))
    gof = next(method for method in value["methods"] if method["method_id"] == "gof")
    assert gof["recipe_status"] == "FROZEN_3K_TRAINING_AUTHORIZED"
    assert gof["common_adapter"]["status"] == "GPU_BUILD_SYNTHETIC_AND_REAL_3K_PACKET_EVALUATOR_PREFLIGHT_PASS"
    assert gof["three_k_qualification_status"] == "QUALIFIED_3K_TRAINING_AUTHORIZED"
    assert gof["three_k_training_allowed"] is True

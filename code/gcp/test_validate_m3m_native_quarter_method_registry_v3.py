...
def test_current_registry_passes_with_exact_gsprior_one_use_gate() -> None:
    ...
    assert result["training_allowed_methods"] == ["gsprior"]
...
def test_completed_qgs_is_not_launchable_while_gsprior_gate_is_open(tmp_path: Path) -> None:
    value = load_registry()
    ...
    assert value["current_one_use_launch_gate"]["method_id"] == "gsprior"
    result = validate_mutation(tmp_path, value)
    assert result["passed"] is True
    assert result["training_allowed_methods"] == ["gsprior"]

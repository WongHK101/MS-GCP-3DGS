...
PIXEL_DOMAIN_RELEASE_SCHEMAS = {RELEASE_V12_SCHEMA, RELEASE_V121_SCHEMA, RELEASE_V122_SCHEMA, RELEASE_V130_SCHEMA}
...
def detect_pixel_domain_release_token(release_base: Path) -> str:
    if (release_base / "v1_3_0_release_file_manifest.json").exists():
        return "v1_3_0"
    ...
    if (release_base / "v1_2_release_file_manifest.json").exists():
        return "v1_2"
    raise FileNotFoundError(f"No supported pixel-domain release manifest found in {release_base}")
...
def validate_release_v12_rows_for_evaluator(
    *,
    ...
    strict_provenance = sidecars.get("release_token") in {"v1_2_1", "v1_2_2", "v1_3_0"}

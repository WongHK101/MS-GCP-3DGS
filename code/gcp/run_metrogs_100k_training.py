...
FORMAL_MANIFEST_FILE_SHA256 = "c2cf9e951d95fee12a28d942e95c5c420df55bc364738b3f8737fed1c78bef3d"
FORMAL_MANIFEST_CANONICAL_SHA256 = "5b4fe34743310bd2225feb2dd236200606be933002fec19d2c9ecb9f3ba6769d"
...
def cleanup_rank_checkpoint(
    *, model_path: Path, rank_checkpoint: Path, merged_checkpoint: Path
) -> dict[str, Any]:
    """Inventory then remove the rank checkpoint duplicated by the merge."""
    model_path = model_path.resolve()
    if rank_checkpoint.is_symlink() or merged_checkpoint.is_symlink():
        raise RuntimeError("symlinked MetroGS checkpoints are forbidden")
    rank_checkpoint = rank_checkpoint.resolve()
    merged_checkpoint = merged_checkpoint.resolve()
    inventory_path = model_path / "rank_checkpoint_inventory_pre_cleanup.json"
    if inventory_path.exists():
        raise FileExistsError(inventory_path)
    for label, path in (("rank", rank_checkpoint), ("merged", merged_checkpoint)):
        if not path.is_file():
            raise RuntimeError(f"missing MetroGS {label} checkpoint: {path}")
        try:
            path.relative_to(model_path)
        except ValueError as exc:
            raise RuntimeError(f"MetroGS {label} checkpoint escaped model root") from exc
    if rank_checkpoint == merged_checkpoint:
        raise RuntimeError("MetroGS rank and merged checkpoints are the same path")
    rank_row = {
        "path": str(rank_checkpoint),
        "bytes": rank_checkpoint.stat().st_size,
        "sha256": sha256(rank_checkpoint),
    }
    merged_row = {
    ...
    inventory = {
        "schema": "m3m_gcp_metrogs_rank_checkpoint_inventory_v1",
        "policy": "remove the hash-inventoried single-GPU rank checkpoint only after merge and PLY conversion pass",
        "rank_checkpoint": rank_row,
        "merged_checkpoint": merged_row,
    }
    descriptor = os.open(
    ...
    if rank_checkpoint.exists():
        raise RuntimeError("MetroGS rank checkpoint survived cleanup")
    if not merged_checkpoint.is_file() or sha256(merged_checkpoint) != merged_row["sha256"]:
        raise RuntimeError("MetroGS merged checkpoint changed during rank cleanup")
    return {
        "inventory_path": str(inventory_path),
        "inventory_sha256": sha256(inventory_path),
        "rank_checkpoint": {**rank_row, "retained_after_merge": False},
        "merged_checkpoint": {**merged_row, "retained_after_merge": True},
        "logical_bytes_removed": rank_row["bytes"],
    }
...
def main() -> int:
    ...
    checkpoint_cleanup = cleanup_rank_checkpoint(
        model_path=model_path,
        rank_checkpoint=rank_checkpoints[0],
        merged_checkpoint=merged_checkpoints[0],
    )
    ...
    payload = {
        ...
        "checkpoint": {
            "rank_path": checkpoint_cleanup["rank_checkpoint"]["path"],
        ...
        "rank_checkpoint_cleanup": {
            "inventory_path": checkpoint_cleanup["inventory_path"],
            "inventory_sha256": checkpoint_cleanup["inventory_sha256"],
            "logical_bytes_removed": checkpoint_cleanup["logical_bytes_removed"],
        },
        "commands": {
            "train": train_command,
            "merge": merge_command,

...
def decide(a100: str, a50: str | None, b100: str | None, b50: str | None) -> dict[str, Any]:
    ...
    if a100 == "PASS" and a50 is None:
        return {"status": "NEEDS_A50", "selected_contract": None, "reason": "candidate_a_100k_pass"}
    if all(status in {"PASS", "GPU_MEMORY_BLOCKED"} for status in a_rows) and "GPU_MEMORY_BLOCKED" in a_rows:
        ...
        if b50 is None:
            return {"status": "NEEDS_B50", "selected_contract": None, "reason": "candidate_b_100k_pass"}
        if b50 != "PASS":
            return {"status": "BLOCKER", "selected_contract": None, "reason": "candidate_b_failure"}
...
def main() -> int:
    ...
    parser.add_argument("--a50", type=Path)
    parser.add_argument("--b100", type=Path)
    parser.add_argument("--b50", type=Path)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    result = decide(
        status_from_path(args.a100), status_from_path(args.a50),
        status_from_path(args.b100), status_from_path(args.b50),
    )
    result.update({
    ...
    return 0 if result["status"] in {"SELECTED_A", "SELECTED_B", "NEEDS_A50", "B_ELIGIBLE", "NEEDS_B50"} else 1

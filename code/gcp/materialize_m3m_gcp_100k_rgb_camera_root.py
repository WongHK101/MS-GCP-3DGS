#!/usr/bin/env python3
"""Create the immutable test-camera loader root for formal 100K RGB evaluation."""
...
def materialize(
    *,
    ...
    test_role = roles.get("test", {})
    if (
        test_role.get("image_count") != EXPECTED_TEST_VIEWS
        or test_role.get("points2d_tracks_present") is not False
        or test_role.get("points3d_bin_present") is not False
        or {
            "cameras.bin": test_role.get("cameras_bin_sha256"),
            "images.bin": test_role.get("images_bin_sha256"),
            "points3D.ply": test_role.get("points3d_ply_sha256"),
        }
        != TEST_SPARSE_SHA
    ):
        raise RuntimeError("formal test role identity mismatch")

    source_root = formal_scene_root / "test"
    source_sparse = source_root / "sparse" / "0"
    source_images = source_root / "images"
    if not source_images.is_dir():
        raise FileNotFoundError(source_images)
    for name, expected in TEST_SPARSE_SHA.items():
        require_sha(source_sparse / name, expected, f"formal test {name}")
    if (source_sparse / "points3D.bin").exists():
        raise RuntimeError("formal test root unexpectedly exposes points3D.bin")

    test_rows = [row for row in formal.get("images", []) if row.get("role") == "test"]
    expected_names = {str(row.get("image_name", "")) for row in test_rows}
    actual_names = {path.name for path in source_images.iterdir() if path.is_file()}
    if (
        len(test_rows) != EXPECTED_TEST_VIEWS
        or len(expected_names) != EXPECTED_TEST_VIEWS
        or actual_names != expected_names
    ):
        raise RuntimeError("formal test image inventory mismatch")
    for row in test_rows:
        path = source_images / str(row["image_name"])
        if path.stat().st_size != int(row["jpeg_bytes"]) or sha256_file(path) != row["jpeg_sha256"]:
            raise RuntimeError(f"formal test RGB identity mismatch: {path.name}")
    ...
    if not all(same_file.values()) or not (output_root / "images").is_symlink():
        raise RuntimeError("RGB camera root did not use the required links")
    ...
    payload: dict[str, Any] = {
        ...
        "status": STATUS,
        "scene": SCENE,
        "purpose": "evaluation-only frozen heldout-camera loader root; never a training, prior or model-selection input",
        ...
        "source_test": {
            ...
            "view_count": EXPECTED_TEST_VIEWS,
            "sparse_sha256": TEST_SPARSE_SHA,
        ...
        "output": {
            ...
            "points3d_bin_purpose": "deterministic empty COLMAP compatibility member; no geometry or tracks",
        },
        "truth_boundary": {
            "heldout_rgb_present_for_metric_and_loader_compatibility_only": True,
            "heldout_rgb_must_not_select_fit_or_optimize_any_parameter": True,
            "graphdeco_loader_may_decode_then_adapter_must_clear_original_image": True,
    ...
    manifest_path = output_root / "RGB_EVALUATION_CAMERA_ROOT_MANIFEST.json"
    ...
    if sha256_file(manifest_path) != sha256_file(evidence_path):
        raise RuntimeError("RGB camera-root manifest/evidence bytes differ")
    return payload

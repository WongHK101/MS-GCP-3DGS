#!/usr/bin/env python3
"""Target-GPU conformance test for PGSR's evaluation-only A/M1/M2/H output."""
...
def raw_peak(all_map: Any) -> dict[str, float | int]:
    value = all_map.detach().cpu().numpy().astype(np.float32)
    if value.shape[0] != 8:
        raise AssertionError(f"patched PGSR all-map must have 8 planes, got {value.shape}")
...
def run(eval_repo: Path) -> dict[str, Any]:
    ...
    variance_check = assert_close(
        "two_variance", variance, 685.7142857142857 / 7.0, atol=2e-4, rtol=2e-5
    )

    return {
        "schema": "m3m_gcp_native_quarter_pgsr_raw_moment_cuda_conformance_v1",
        ...
        "native_plane_sources": ["A"],
        "evaluation_only_weighted_planes": ["M1", "M2", "H"],

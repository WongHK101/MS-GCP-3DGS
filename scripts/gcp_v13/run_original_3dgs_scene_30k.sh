...
: "${SCENE:?Set SCENE to a frozen v1.3 scene id}"
...
DATASET_ROOT=/root/autodl-tmp/datasets/ms-gcp-v13/$RELEASE_DIGEST/$SCENE
RELEASE_ROOT=/root/autodl-tmp/datasets/ms-gcp-v13/$RELEASE_DIGEST/release_v1_3_0
ORCH_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
BUILD_ROOT=/root/autodl-tmp/build/ms-gcp-v13/3dgs-original/$METHOD_COMMIT/$SCENE/$RUN_ID
RUN_ROOT=/root/autodl-tmp/runs/ms-gcp-v13/3dgs-original/$SCENE/$RUN_ID
MODEL_ROOT=$RUN_ROOT/02_checkpoints/model
RECIPE=$ORCH_ROOT/configs/gcp_v13_original_3dgs_six_scene_recipe_v1.json
...
"$ENV_ROOT/bin/python" "$ORCH_ROOT/code/gcp/validate_gcp_v13_training_mirror.py" \
  --root "$DATASET_ROOT" \
  --recipe "$RECIPE" \
  --report "$BUILD_ROOT/preflight/training_mirror_validation.json"
...
cp "$BUILD_ROOT/preflight/training_mirror_validation.json" "$RUN_ROOT/00_preflight/"
...
test -z "$(find "$DATASET_ROOT" "$RELEASE_ROOT" -perm /222 -print -quit)"
(cd "$DATASET_ROOT" && sha256sum -c SOURCE_MANIFEST.sha256)

SOURCE_PRE=$(source_digest "$DATASET_ROOT")
printf '%s\n' "$SOURCE_PRE" > "$RUN_ROOT/00_preflight/dataset_tree_digest_before.txt"
sha256sum "$DATASET_ROOT/SOURCE_MANIFEST.json" "$DATASET_ROOT/SOURCE_MANIFEST.sha256" > "$RUN_ROOT/00_preflight/source_manifest.sha256"

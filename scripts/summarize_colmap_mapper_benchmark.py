...
def select_largest_model(sparse_root: Path) -> tuple[Path, dict]:
    candidates = []
    ...
    _, _, path, cameras, images, points = max(candidates, key=lambda row: row[:2])
...
def build_summary(args: argparse.Namespace) -> dict:
    ...
    candidate_model, candidate_stats = select_largest_model(
        candidate_run / "RGB" / "distorted" / "sparse"
    )
    baseline_model, baseline_stats = select_largest_model(
